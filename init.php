...
// define __autoload to automatically look in the omega class dir, and API include directories if available
spl_autoload_register(function ($class_name) {
    ...
    // if omega has been loaded then add the API's class dirs too
    if ($om !== null) {
...
$api_name = getenv('OMEGA_SERVICE');
if ($api_name == '' || $api_name === false) {
    // no API name set? peek at the URI and figure it out from there then
    $uri = null;
    ...
    // the first part (e.g. /api, /foo/bar/api, should be the API location)
    $location = trim(preg_replace('/\/+/', '/', $uri), '/');
    // see if it matches any of our config files
    $shed = new OmegaFileShed(OmegaConstant::data_dir());
    $server_config = $shed->get('OmegaServer', 'apis');
    foreach ($server_config['apis'] as $name => $enabled) {
        if ($enabled) {
            $config = $shed->get($name, "config");
            $api_loc = trim(@$config['omega']['location'], '/');
            if (strpos($location, $api_loc) === 0) {
                $api_name = $name;
                break;
            }
        }
