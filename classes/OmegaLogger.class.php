...
class OmegaLogger extends OmegaRESTful {
    private $buffer;
    private $log;
    private $data;
    private $log_dir;
    private $log_aborted;
    private $verbosity = array();
    private $verbose_opts = array(
        'api' => 'The API being executed', // automatically included if anything else set
    ...
    public function __construct() {
        $om = Omega::get();
        $this->log_dir = OmegaConstant::data_dir()
            . '/' . $om->api_name . '/logs';
        if (! OmegaLib::mkdir($this->log_dir, 0700, true)) {
            throw new Exception("Failed to create log directory '$this->log_dir'.");
        }
        $this->buffer = '';
        $this->log = array();
        $this->data = array();
        $this->log_aborted = false;
        // set our default verbosity level
        $this->_set_verbosity = $om->config->get('omega/logger/verbosity', array());
    }
    ...
    public function commit_log($successful = true, $verbosity = null) {
        ...
        OmegaLib::mkdir($log_path, 0700, true);

        // any verbosity at all? write out a basic log header
        if ($verbosity) {
            ...
            if ($om->config->get('omega/')) {
                $this->buffer_write(' - ' . $om->auth->authed_username);
            }
        ...
        if ($file_handle === false) {
            throw new Exception("Failed to open API log file '$log_file'.");
        }
        if (flock($file_handle, LOCK_EX) === false) {
            throw new Exception("Failed to lock API log file '$log_file'.");
        }
        if (fwrite($file_handle, $this->buffer) === false) {
            throw new Exception("Failed to write API log contents to '$log_file'.");
        }
        // close ourselves
    ...
    public function list_log_files() {
        ...
        if (! is_dir($log_path)) {
            throw new Exception('There are no log files for API ' . $om->api_name . '.');
        }
