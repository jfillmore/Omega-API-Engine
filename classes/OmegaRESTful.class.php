...
abstract class OmegaRESTful {
    ...
    */
    ...
    public function _sorted_routes($routes = null) {
        ...
        foreach ($routes as $route => $target) {
            if ($route == '@pre_route') {
                ...
                $route = OmegaLib::pretty_path($route, true);
    ...
    public function _route($path, $params = array()) {
        ...
        $path = OmegaLib::pretty_path($path, true);
            ...
            foreach ($this->_sorted_routes() as $route => $target) {
                ...
                if ($route === '@pre_route') {
                    ...
                    if ($parsed) {
                        ...
                        // unless we're just querying for options -- skip it
                        if ($pre_route && ! $is_query) {
        ...
        // the _introspect method will take of checking for any matching end-points and validating the remaining path
        if ($is_query) {
            $params['api_path'] = $path;
            return array(
                'api_branch' => $this,
                'endpoint' => '_introspect',
                'target' => '_introspect',
                'params' => $params
            );
        }
        $handler = $this->_parse_handlers($path, null, $params);
        if ($pre_route && ! $is_query) {
            // FIXME: get_api_params() will return params collected already; we don't want to pass those twice
            $this->$pre_route(
                $_SERVER['REQUEST_METHOD'],
    ...
    public function _parse_handlers($path, $method = null, $params = array()) {
        $om = Omega::get();
        $debug = $om->config->get('omega/debug_routes', false);
        if ($method === null) {
        ...
        // this will trigger a 404 during API routing
        return array(
            'api_branch' => $this,
            'endpoint' => null,
            'target' => null,
    ...
    public function _introspect($api_path = '/', $recurse = false, $verbose = false) {
        $om = Omega::get();
        $path = OmegaLib::pretty_path($api_path, true);
        // just asking about this branch, or gotta match a path?
        if ($path == '/') {
            return $om->request->get_branch_info($this, $recurse, $verbose);
        } else {
            // otherwise we need to see what potential end-points reside at this path
            $handlers = array();
            $found = 0; // make sure we found at least one handler, or it's just a bad route
            foreach ($this->_sorted_handlers() as $h_method => $list) {
                $handlers[$h_method] = array();
                $handler = $this->_parse_handlers($api_path, $h_method);
                if ($handler['target'] !== null) {
                    ...
                    $handlers[$h_method][$api_path] = $om->request->get_method_info(
                        $r_method,
                        $verbose
                    );
                }
            ...
            if (! $found) {
                ...
                throw new Exception("Invalid API path: " . $om->request->get_api(true));
            }
    ...
    private function _parse_path($path, $route, $partial_route = false) {
        ...
        */
        ...
        $path_str = OmegaLib::pretty_path($path, true);
        $path = explode('/', substr($path_str, 1));
        $route_str = OmegaLib::pretty_path($route, true);
        ...
        if ($return['params']) {
            $om->request->add_api_params($return['params']);
        }
        return $return;
    }
}
