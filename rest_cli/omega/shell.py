...
class Shell:
    ...
    def __init__(self, argv):
        ...
        self.args = self.parse_args(argv, self.args)
        self.client = client.OmegaClient(self.args['url']);
    ...
    def print_help(self, shell = False):
        sys.stderr.write(
'''usage: om_api HTTP-Verb|Command API [API_PARAMS] [ARGUMENTS]
...
OTHER OPTIONS (may also be set via 'set' command)
   ...
   -r, --raw              Print response data in raw form (repeat for unformatted JSON).
   -rr, --rawraw          Same as '--raw' twice.
   -v, --verbose          Print verbose debugging info to stderr.
   -c, --color            Colorize return output (unless returning raw data).
   -h, --help             This information.
   -u, --url URL          A URL to the service location (default: https://localhost/).
   -j, --json             Use next argument as JSON-encoded list of args.
   -s, --shell            Shell mode for multiple APIs within a session.

API_PARAMS
   foo=bar                Set 'foo' to 'bar' (always uses strings)
   foo.bar=3              Array param '{"foo": {"bar": "3"}}'
   foo:='{"bar":3}'       JSON-encoded value
    
Environmentals:
   API_USER               The username to authenticate with.
   API_PASS               The password to authenticate with.
   API_TOKEN              The token (e.g. in lieu of the user/pass) to authenticate with.
...
''')
    ...
    def parse_args(self, cmd, arg_slice = None):
        # our API info
        args = {
            'api': None,
            'cmd': None,
        ...
        while i < len(parts):
            ...
            if len(part) == 0:
                ...
                try:
                    args['api_args'] = self.decode(parts[i])
                except Exception, e:
                    sys.stderr.write('Invalid JSON:' + e.message)
                    return
            elif part == '-f' or part == '--full':
                args['full_response'] = True
            elif part == '-r' or part == '--raw' or part == '--rawraw' or part == '-rr':
                #  twice means no formatting
                if part == '-rr' or part == '--rawraw' or args['raw_response']:
                ...
                if args['cmd'] == None:
                    args['cmd'] = part.lower()
                    # process any aliases
                    if args['cmd'] in self.api_aliases:
                        args['cmd'] = self.api_aliases[args['cmd']]
                elif args['cmd'] in self.api_cmds and not args['api']:
    ...
    def parse_cmd(self, cli_cmd):
        ...
        if args['cmd'] == None or len(args['cmd']) == 0:
            # no command, just do nothing
            return {
                'response': None,
                'result': True
            }
        elif args['cmd'] in self.api_cmds:
            # run an API
            try: 
                ...
                result = False
                response = e.message
            self.last_rv = int(not result)
            # prep response redirection, since it worked
            if args['stdout_redir'] != None:
                try:
            ...
            try:
                self.run_cmd(args['cmd'], args['cmd_args'])
                response = None
                if response == False:
                    return False
                result = True
